/*
# Exclusively from Vizu project 
# Do not use this fore any commercial thing
# If you abuse that bot we will kick you from bot 
# Do not edit (Respect to the Devaoloper) 
# All rights reserved ©Recon
*/
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    conn.on('open', async () => {
        ...
        if (config.WORKTYPE == 'public') {
            if (config.LANG == 'EN' || config.LANG == 'SI') {
                ...
                if (conn.user.jid === '94782060439@s.whatsapp.net' ) {
    ...
    conn.on('chat-update', async m => {
        ...
        if (config.SUPPORT == '94782060439-1616997271') {     
            var sup = config.SUPPORT.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sup.includes(msg.key.remoteJid.split('@')[0]) : sup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.SUPPORT2 == '94782060439-1619710622') {     
            var tsup = config.SUPPORT2.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G1 == '94757783040-1538284375') {     
            var tsup = config.G1.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        if (config.G2 == '94757783040-1553586710') {     
            var tsup = config.G2.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G3 == '94757783040-1563444031') {     
            var tsup = config.G3.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G4 == '94757783040-1563796118') {     
            var tsup = config.G4.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G5 == '94757783040-1574127271') {     
            var sup = config.G5.split(',');                            
            if(msg.key.remoteJid.includes('-') ? sup.includes(msg.key.remoteJid.split('@')[0]) : sup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G6 == '94757783040-1577356855') {     
            var tsup = config.G6.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G7 == '94782060439-1606916704') {     
            var tsup = config.G7.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G8 == '94782060439-1612721726') {     
            var tsup = config.G8.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G9 == '94782060439-1603603121') {     
            var tsup = config.G9.split(',');                            
            if(msg.key.remoteJid.includes('-') ? tsup.includes(msg.key.remoteJid.split('@')[0]) : tsup.includes(msg.participant ? msg.participant.split('@')[0] : msg.key.remoteJid.split('@')[0])) return ;
        }
        
        if (config.G10 == '94782060439-1603611085') {     
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    if ((config.fromMe === false && command.fromMe === true &&
                    ...
                    if (sendMsg) {
                        ...
                        try {
                            await command.function(whats, match);
                            ...
                            else if (config.LANG == 'EN' || config.LANG == 'SI') {
                                await conn.sendMessage(conn.user.jid, '*-- ERROR REPORT [Recon 🇱🇰] --*' + 
                                    '\n*Recon getting an error!*'+
                                    '\n_This error log may include your number or the number of an opponent. Please be careful with it!_' +
                                    '\n_Aslo you can join our support group:_ https://chat.whatsapp.com/CSNok5JOi1W9yOsmPLyFng' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [Recon 🇱🇰] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [Recon 🇱🇰] ⚕️*' + 
                                        '\n========== ```Error Read!``` ==========' +
                                        '\n\n*I'm so:* _Deleting Plugin_' +
                                        '\n*Reason:* _Incorrect entry of the plugin name to be deleted._' +
                                        '\n*Solution:* _Please click on the plugin you want to delete_ *__* _try without. If you still get the error_ ```?(.*) / $``` _Enter complete expressions like._'
                                        , MessageType.text
                                    );
                                }
                                else {
                                ...
                                if (error.message.includes('URL')) {
                                    ...
                                    return await conn.sendMessage(conn.user.jid, '*⚕️ ERROR ANALYSIS [WRecon 🇱🇰] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                        '\n\n*Main Error:* _Reply Delete Function_' +
                                        '\n*Reason:* _Using IMG or Wiki commands._' +
                                        '\n*Solution:* _There is no solution for this error. It is not a fatal error._'
